...
export function InnovationSection() {
  return (
    <section
      id="diagnostico"
      className="bg-[linear-gradient(135deg,#1B4D8A_0%,#0E5A47_54%,#0B6B57_100%)] px-5 py-24 text-white lg:px-8"
    >
          ...
          <div className="grid gap-6 lg:grid-cols-5">
            {innovationItems.map((item, index) => (
              <Reveal key={item.title} delay={index * 0.08}>
                <article className="relative h-full rounded-lg border border-forest-900/12 bg-white p-6 pl-20 text-forest-950 shadow-[0_24px_80px_rgba(14,90,71,0.18)] transition duration-300 hover:-translate-y-1 hover:border-gold-500/45 hover:shadow-[0_28px_90px_rgba(14,90,71,0.24)] lg:pl-6 lg:pt-20">
                  <div className="absolute left-0 top-6 flex h-12 w-12 items-center justify-center rounded-full border border-gold-500 bg-white text-gold-500 shadow-glow lg:left-6 lg:top-0">
                  ...
                  <p className="mt-4 text-sm leading-7 text-forest-900/78">
                    {item.description}
                  </p>
                </article>
              </Reveal>
